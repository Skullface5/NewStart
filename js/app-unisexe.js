 ...
 function paintCachedProducts() {
   try {
     const raw = localStorage.getItem('rosa_cache_unisexe');
     if (!raw) return false;
...
async function loadProducts() {
        ...
        try {
          const { data, error } = await supabase.from('products').select('*').eq('category', 'unisexe').order('created_at', { ascending: true });
          ...
          populateBrandFilter();
          applyFiltersAndSort();
 rosaSaveCache('rosa_cache_unisexe', products);
        } catch (err) {
          console.error(err);
