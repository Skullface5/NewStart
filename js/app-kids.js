 ...
 function paintCachedProducts() {
   try {
     const raw = localStorage.getItem('rosa_cache_kids');
     if (!raw) return false;
...
async function loadProducts() {
        ...
        try {
          const { data, error } = await supabase.from('products').select('*').eq('category', 'kids').order('created_at', { ascending: true });
          ...
          populateBrandFilter();
          applyFiltersAndSort();
 rosaSaveCache('rosa_cache_kids', products);
        } catch (err) {
          console.error(err);
