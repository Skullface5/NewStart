    ...
    (function () {
      ...
      const translations = {
        fr: {
          ...
          womenCat: 'Parfums Femme', womenDesc: 'Des fragrances florales, sensuelles et élégantes qui célèbrent votre féminité.',
        ...
        en: {
          ...
          womenCat: 'Women Perfumes', womenDesc: 'Floral, sensual and elegant fragrances that celebrate your femininity.',
        ...
        ar: {
          ...
          womenCat: 'عطور نسائية', womenDesc: 'عطور زهرية وحسية وأنيقة تحتفي بأنوثتك.',
 ...
 function paintCachedProducts() {
   try {
     const raw = localStorage.getItem('rosa_cache_women');
     if (!raw) return false;
...
async function loadProducts() {
        ...
        try {
          const { data, error } = await supabase.from('products').select('*').eq('category', 'women').order('created_at', { ascending: true });
          ...
          populateBrandFilter();
          applyFiltersAndSort();
 rosaSaveCache('rosa_cache_women', products);
        } catch (err) {
          console.error(err);
